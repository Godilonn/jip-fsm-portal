/**
 * @file features/logistics/components/ImportAtkModal.tsx
 * @description Modal import massal ATK (Alat Tulis Kantor) dari file Excel / CSV.
 *
...
async function parseFile(file: File): Promise<{ rows: ParsedRow[]; headers: string[] }> {
  ...
  const rows: ParsedRow[] = raw.map((r) => {
    const mapped: Partial<ParsedRow> = {
      kode: "", namaBarang: "", kategori: "Lainnya", stok: 0,
    };
    ...
    for (const [key, val] of Object.entries(r)) {
      ...
      if (!field) continue;
      if (field === "stok") {
        mapped.stok = parseInt(String(val).replace(/[^0-9]/g, "")) || 0;
      } else {
...
function downloadTemplate() {
  ...
  const sample = [
    ["ATK-001", "Pulpen Pilot", "Alat Tulis", "10"],
    ["ATK-002", "Kertas HVS A4 80gsm", "Kertas", "5"],
    ["ATK-003", "Amplop Coklat Besar", "Pengiriman", "20"],
  ];
  ...
  a.href = url;
  a.download = "template-import-atk.csv";
  a.click();
...
export default function ImportAtkModal({ isOpen, onClose, onImported }: Props) {
  ...
  const handleImport = async () => {
    ...
    setLoading(true);
    try {
      const resp = await fetch("/api/inventory/atk/import", {
        method: "POST",
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Upload file Excel atau CSV untuk import data ATK sekaligus</p>
          </div>
            ...
            <div className="space-y-5">
              ...
              <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4 flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-indigo-600 mt-0.5">
                    Isi data ATK di template, lalu upload di sini. Kolom <strong>Nama Barang</strong> wajib diisi.
                  </p>
                </div>
                <button
              ...
              <div className="bg-slate-50 rounded-xl p-4 text-xs text-slate-600">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {["Kode", "Nama Barang *", "Kategori", "Stok"].map(c => (
          ...
          {step === 2 && (
            ...
            >
              {loading ? (
                ...
                <><Upload size={14} /> Import {validCount} Data ATK</>
              )}
            </button>
          )}
          {step === 3 && (
